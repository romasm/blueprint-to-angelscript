...
The BlueprintExporter plugin runs an HTTP server on port 7233 inside the running Unreal Editor.
...
If the editor is not responding, ask the user to open Unreal Editor with the SignalsGame project.
...
All endpoints are on `http://localhost:7233`. Responses return `output_path` — you must then read the JSON file at that path.
...
```bash
curl -s "http://localhost:7233/export-struct?path=/Game/Path/To/MyStruct"
```
Then read the JSON file at `output_path`.
...
```bash
curl -s "http://localhost:7233/export-enum?path=/Game/Path/To/MyEnum"
```
Then read the JSON file at `output_path`.
